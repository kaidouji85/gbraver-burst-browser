...
test("Tween Durationがスケールされている", () => {
  const t1 = new TWEEN.Tween({}).to({}, 400);
...
test("チェインしたTween Durationがスケールされている", () => {
  const t1 = new TWEEN.Tween({}).to({}, 500);
  const t2 = new TWEEN.Tween({}).to({}, 300);
  t1.chain(t2);
  scaleTweenDuration(t1, 0.5);
  /* eslint-disable @typescript-eslint/no-explicit-any */
...
test("ループしたTweenでもDurationがスケールされている", () => {
  const t1 = new TWEEN.Tween({}).to({}, 600);
  const t2 = new TWEEN.Tween({}).to({}, 400);
  t1.chain(t2);
  t2.chain(t1);
  scaleTweenDuration(t1, 0.5);
  /* eslint-disable @typescript-eslint/no-explicit-any */
...
test("複雑にチェインしたTweenでもDurationがスケールされている", () => {
  const t1 = new TWEEN.Tween({}).to({}, 100);
  const t2 = new TWEEN.Tween({}).to({}, 200);
  const t3 = new TWEEN.Tween({}).to({}, 300);
  const t4 = new TWEEN.Tween({}).to({}, 400);
  const t5 = new TWEEN.Tween({}).to({}, 500);
