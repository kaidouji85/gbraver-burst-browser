...
test("ヒットする最小バッテリーを正しく計算できる", () => {
  expect(getMinimumBatteryToHitOrCritical(attacker, defender, 2)).toBe(3);
});

test("クリティカルする最小バッテリーを正しく計算できる", () => {
  expect(getMinimumBatteryToHitOrCritical(attacker, defender, 0)).toBe(1);
});

test("ヒット、クリティカルできない場合はnullを返す", () => {
  expect(getMinimumBatteryToHitOrCritical(attacker, defender, 5)).toBe(null);
});