...
test("ステージクリアの処理が正しい", () => {
  ...
  expect(
    updateNPCBattleState(state, { type: "GameOver", winner: player.playerId })
...
test("ステージミスの処理が正しい", () => {
  const state = { player, stages, stageIndex: 0 };
  expect(
    updateNPCBattleState(state, { type: "GameOver", winner: "not-player" })
  ).toEqual({ state, result: "StageMiss" });
});

test("引き分けはステージミスとみなす", () => {
  const state = { player, stages, stageIndex: 0 };
  expect(updateNPCBattleState(state, { type: "EvenMatch" })).toEqual({
    state,
    result: "StageMiss",
  });
});
