...
export const pilotIcon: DOMStubStory = domStub(resources => {
  ...
  icon.selectedNotifier().subscribe(() => {// NOP
  });
  return icon.getRootHTMLElement();
});
export const armdozerSelector: DOMStubStory = domStub(resources => {
  const armdozerIds = [ArmDozerIds.NEO_LANDOZER, ArmDozerIds.SHIN_BRAVER, ArmDozerIds.WING_DOZER, ArmDozerIds.LIGHTNING_DOZER];
