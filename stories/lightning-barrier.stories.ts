...
const lightningBarrierStory = (
  ...
  const stub = new TDGameObjectStub(({ resources, gameObjectAction }) => {
    const barrier = new LightningBarrierGameEffect(resources, gameObjectAction);
    fn(barrier);
    return {
...
/** 表示->非表示 */
export const showHidden = lightningBarrierStory((barrier: LightningBarrierGameEffect) => {
  delay(1000)
    .chain(barrier.show())
    .chain(delay(2000))
