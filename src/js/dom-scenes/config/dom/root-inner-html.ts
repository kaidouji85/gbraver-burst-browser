import type { BattleAnimationTimeScale, GbraverBurstBrowserConfig, WebGLPixelRatio } from "../../../game/config/browser-config";
import { BattleAnimationTimeScales, WebGLPixelRatios } from "../../../game/config/browser-config";
...
export function rootInnerHTML(ids: DataIDs, config: GbraverBurstBrowserConfig): string {
  ...
  const battleAnimationTimeScaleOptions = BattleAnimationTimeScales.map(v => battleAnimationTimeScaleOption(v)).reduce((a, b) => a + b);
  ...
  const webGLPixelRatioOptions = WebGLPixelRatios.map(v => webGLPixelRatioOption(v)).reduce((a, b) => a + b);
  return `
