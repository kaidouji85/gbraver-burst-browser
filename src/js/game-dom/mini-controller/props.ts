...
export function createMiniControllerProps(): MiniControllerProps {
  ...
  const batteryButtons = R.times(R.identity, MAX_BATTERY_BUTTON).map(battery => new BatteryButton(battery));
  batteryButtons.forEach(batteryButton => {
    elements.batteries.appendChild(batteryButton.getRootHTMLElement());
  });
  const batteryPush = merge(...batteryButtons.map(batteryButton => batteryButton.pushNotifier()));
  return {
    ...elements,
    root,
    batteryButtons,
