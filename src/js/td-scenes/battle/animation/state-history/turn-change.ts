...
export function turnChangeAnimation(props: StateAnimationProps, gameState: GameStateX<TurnChange>): Animate {
  ...
  const turnStart = activeHUDPlayer.turnStart.show().chain(delay(400)).chain(activeHUDPlayer.turnStart.hidden());
  const forTurnChange = empty().chain(delay(800), turnStart, activeHUDPlayer.gauge.battery(activeStatus.armdozer.battery), activeTDPlayer.recoverBattery.show(turnChange.recoverBattery).chain(delay(400)).chain(activeTDPlayer.recoverBattery.hidden()));
  const forContinuousAttack = empty().chain(delay(800), turnStart, process(() => {
