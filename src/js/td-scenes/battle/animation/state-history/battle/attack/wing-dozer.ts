...
function focusToAttacker(camera: TDCamera, attacker: WingDozer): Animate {
  const duration = 400;
  return all(track(camera, attacker.getObject3D().position.x * 0.6, duration), dolly(camera, "-20", duration));
}
...
function miss(param: WingDozerBattle<Miss>): Animate {
  return param.attackerSprite.charge().chain(delay(600)).chain(param.attackerSprite.upper()).chain(param.defenderSprite.avoid()).chain(delay(500)).chain(param.attackerSprite.upperToStand()).chain(delay(500));
}
