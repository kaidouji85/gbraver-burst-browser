 ...
 * @return 抽出結果
 */
export function extractUnloadedResorceConfigs(configs: Configs, resources: Resources): Configs {
  ...
  return {
    gltfConfigs: configs.gltfConfigs.filter(v => !loadedGltfIDs.includes(v.id)),
    textureConfigs: configs.textureConfigs.filter(v => !loadedTextureIDs.includes(v.id)),
    cubeTextureConfigs: configs.cubeTextureConfigs.filter(v => !loadedCubeTextureIDs.includes(v.id)),
    canvasImageConfigs: configs.canvasImageConfigs.filter(v => !loadedCanvasImageIDs.includes(v.id)),
    soundConfigs: configs.soundConfigs.filter(v => !loadedSoundIDs.includes(v.id))
  };
}