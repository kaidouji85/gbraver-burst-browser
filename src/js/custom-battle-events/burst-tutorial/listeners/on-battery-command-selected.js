...
export async function onBatteryCommandSelected(props: $ReadOnly<BatteryCommandSelected>, state: BurstTutorialState): Promise<Ret> {
  ...
  if (!isPlayerFullBattery && enableBurst) {
    ...
    return {state: {...state, isLoseIfNoDefense5Complete: true, selectableCommands: 'BurstOnly'}, cancel: {isCommandCanceled: true}};
  }
  ...
  if (!isPlayerFullBattery && !enableBurst && enablePilotSkill) {
    ...
    return {state: {...state, selectableCommands: 'PilotSkillOnly', isLoseIfNoDefense5Complete: true}, cancel: {isCommandCanceled: true}};
  }

  if (!isPlayerFullBattery && !enableBurst && !enablePilotSkill) {
    await defense5(props, state);
    await canNotChangeBattery(props);
    return {state: {...state, isLoseIfNoDefense5Complete: true}, cancel: {isCommandCanceled: false}};
  }

  if (isPlayerFullBattery) {
    await defense5(props, state);
    state.isLoseIfNoDefense5Complete ? await notDefense5Carelessly(props) : await redoBatterySelect(props);
    return {state: {...state, isLoseIfNoDefense5Complete: true}, cancel: {isCommandCanceled: true}};
  }
