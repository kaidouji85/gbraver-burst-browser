...
export class PlayerDamageIndicatorView implements DamageIndicatorView {
  ...
  constructor(resources: Resources) {
    ...
    const damageNumber = damageNumberResource ? damageNumberResource.texture : new THREE.Texture();
    this.#numbers = R.times(() => new HorizontalAnimationMesh({
  ...
  engage(model: DamageIndicatorModel): void {
    const values: number[] = String(model.damage).split("").map(v => Number(v));
    this.#numbers.forEach((mesh, meshIndex) => {
      mesh.setOpacity(0);
      values.filter((value, valueIndex) => meshIndex === valueIndex).forEach(value => {
