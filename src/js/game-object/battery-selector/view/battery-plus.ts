...
export class BatteryPlus {
  ...
  constructor(param: Param) {
    const active = param.resources.canvasImages.find(v => v.id === CANVAS_IMAGE_IDS.BATTERY_PLUS)?.image ?? new Image();
  ...
  update(model: BatterySelectorModel): void {
    ...
    this.#activeButton.getObject3D().scale.set(model.plusButtonScale, model.plusButtonScale, model.plusButtonScale);
    ...
    this.#buttonDisabled.getObject3D().scale.set(model.plusButtonScale, model.plusButtonScale, model.plusButtonScale);
  }
