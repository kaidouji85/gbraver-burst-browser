...
export class BatteryButton {
  ...
  constructor(param: Param) {
    ...
    const button = param.resources.canvasImages.find(v => v.id === CANVAS_IMAGE_IDS.BATTERY_BUTTON)?.image ?? new Image();
    ...
    const attackLabel = param.resources.canvasImages.find(v => v.id === CANVAS_IMAGE_IDS.BATTERY_LABEL_ATTACK)?.image ?? new Image();
    ...
    const defenseLabel = param.resources.canvasImages.find(v => v.id === CANVAS_IMAGE_IDS.BATTERY_LABEL_DEFENSE)?.image ?? new Image();
    ...
    const currentBattery = param.resources.textures.find(v => v.id === TEXTURE_IDS.BATTERY_CURRENT_VALUE)?.texture ?? new THREE.Texture();
