...
export class PlayerGenesisBraverView implements GenesisBraverView {
  ...
  engage(model: GenesisBraverModel): void {
    ...
    const currentStandardMesh = this.#meshes.find(v => v.meshType === "STANDARD" && v.animationType === model.animation.type);
    ...
    const currentOutlineMesh = this.#meshes.find(v => v.meshType === "OUTLINE" && v.animationType === model.animation.type);
