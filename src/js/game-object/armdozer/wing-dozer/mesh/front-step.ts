 ...
 * @return メッシュ
 */
export function wingDozerActiveFrontStep(resources: Resources): ArmdozerAnimation {
  const texture = resources.textures.find(v => v.id === TEXTURE_IDS.WING_DOZER_FRONT_STEP)?.texture ?? new THREE.Texture();
