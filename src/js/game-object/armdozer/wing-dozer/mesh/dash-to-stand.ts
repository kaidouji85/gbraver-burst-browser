 ...
 * @return メッシュ
 */
export function wingDozerActiveDashToStand(resources: Resources): ArmdozerAnimation {
  const texture = resources.textures.find(v => v.id === TEXTURE_IDS.WING_DOZER_DASH_TO_STAND)?.texture ?? new THREE.Texture();
