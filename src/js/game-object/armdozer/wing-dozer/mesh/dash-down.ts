 ...
 * @return メッシュ
 */
export function wingDozerActiveDashDown(resources: Resources): ArmdozerAnimation {
  const texture = resources.textures.find(v => v.id === TEXTURE_IDS.WING_DOZER_DASH_DOWN)?.texture ?? new THREE.Texture();
