...
/**
 * ネオラインドーザアクティブガードメッシュ生成
 * @param resources リソース管理オブジェクト
 * @return メッシュ
 */
export function neoLandozerActiveGuard(resources: Resources): ArmdozerAnimation {
  const texture = resources.textures.find(v => v.id === TEXTURE_IDS.NEO_LANDOZER_GUARD)?.texture ?? new THREE.Texture();
