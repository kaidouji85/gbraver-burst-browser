 ...
 * @return メッシュ
 */
export function lightningDozerGutsToStand(resources: Resources): ArmdozerAnimation {
 ...
 * @return メッシュ
 */
export function lightningDozerActiveGutsToStand(resources: Resources): ArmdozerAnimation {
  const texture = resources.textures.find(v => v.id === TEXTURE_IDS.LIGHTNING_DOZER_GUTS_TO_STAND)?.texture ?? new THREE.Texture();
