 ...
 * @return メッシュ
 */
export function lightningDozerGutsDown(resources: Resources): ArmdozerAnimation {
 ...
 * @return メッシュ
 */
export function lightningDozerActiveGutsDown(resources: Resources): ArmdozerAnimation {
  const texture = resources.textures.find(v => v.id === TEXTURE_IDS.LIGHTNING_DOZER_GUTS_DOWN)?.texture ?? new THREE.Texture();
