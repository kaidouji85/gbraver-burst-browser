 ...
 * @return メッシュ
 */
export function lightningDozerFrontStep(resources: Resources): ArmdozerAnimation {
 ...
 * @return メッシュ
 */
export function lightningDozerActiveFrontStep(resources: Resources): ArmdozerAnimation {
  const texture = resources.textures.find(v => v.id === TEXTURE_IDS.LIGHTNING_DOZER_FRONT_STEP)?.texture ?? new THREE.Texture();
