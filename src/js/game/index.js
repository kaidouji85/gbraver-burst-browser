...
export class Game {
  ...
  constructor(param: Param) {
    ...
    const suddenlyBattleEnd = this.#props.suddenlyBattleEnd
      .stream()
      .chain(map(() => ({ type: "SuddenlyBattleEnd" })));
    const webSocketAPIError = createStream(
      this.#props.api.websocketErrorNotifier()
    ).chain(map((error) => ({ type: "WebSocketAPIError", error })));
    const WebSocketAPIUnintentionalClose = createStream(
      this.#props.api.websocketUnintentionalCloseNotifier()
    ).chain(
      map((error) => ({ type: "WebSocketAPIUnintentionalClose", error }))
    );
