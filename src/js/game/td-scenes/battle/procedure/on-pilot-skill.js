// @flow
import {all} from "../../../../animation/all";
import {delay} from "../../../../animation/delay";
import type {DoPilotSkill} from "../actions/do-pilot-skill";
import type {BattleSceneProps} from "../battle-scene-props";
import {playAnimation} from "../play-animation";
import {toCustomBattleEventProps} from "../to-custom-battle-event-props";
import {progressGame} from "./progress-game";

/**
 * パイロットスキル発動時の処理
 * 
 * @param props 戦闘シーンプロパティ 
 * @param action パイロットスキル発動アクション
 * @return 処理が完了したら発火するPromise
 */
export async function onPilotSkill(props: BattleSceneProps, action: DoPilotSkill): Promise<void> {
  await props.exclusive.execute(async () => {
    action.event.stopPropagation();
    const pilotSkillCommand = {type: 'PILOT_SKILL_COMMAND'};
    const {isCommandCanceled} = props.customBattleEvent
      ? await props.customBattleEvent.onPilotSkillCommandSelected({...toCustomBattleEventProps(props), pilot: pilotSkillCommand})
      : {isCommandCanceled: false};
    if (isCommandCanceled) {
      return;
    }
    await playAnimation(
      all(
        props.view.hud.gameObjects.pilotButton.decide(),
        props.view.hud.gameObjects.burstButton.close(),
        props.view.hud.gameObjects.batterySelector.close(),
        props.view.hud.gameObjects.timeScaleButton.close(),
      )
        .chain(delay(500))
        .chain(props.view.hud.gameObjects.pilotButton.close())
    , props);
    await progressGame(props, pilotSkillCommand);
  });
}